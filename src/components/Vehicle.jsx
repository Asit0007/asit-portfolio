...
// ── Camera constants ─────────────────────────────────────────────────────────
// Default follow cam — behind car (-Z = camera, car faces +Z)
const CAM_OFFSET    = new THREE.Vector3(-8, 18, -20)
const CAM_LERP      = 3.5
const CAM_LERP_ZONE = 1.8   // slower transition into zone view

// Zone birds-eye camera targets — defined once, never mutated
const ZONE_CAMS = {
  cloud:    { pos: new THREE.Vector3(0, 34, -55),  look: new THREE.Vector3(0, 0.5, -55)  },
  projects: { pos: new THREE.Vector3(55, 34, 0),   look: new THREE.Vector3(55, 0.5, 0)   },
  hobbies:  { pos: new THREE.Vector3(-55, 34, 0),  look: new THREE.Vector3(-55, 0.5, 0)  },
}

const HAS_GLTF = true // set true + add public/models/car.glb to use GLTF
...
function BoxCar() {
  return (
    <>
      ...
      {/* Wheels */}
      {[[-0.95,-0.18,1.1],[0.95,-0.18,1.1],[-0.95,-0.18,-1.1],[0.95,-0.18,-1.1]].map(([x,y,z],i) => (
...
function VehicleInner(props, ref) {
  ...
  const lastSpeed  = useRef(0)    // for collision detection
  const prevBrake  = useRef(false) // for brake sound trigger
  ...
  useFrame((state, delta) => {
    ...
    // Brake sound — trigger on key DOWN only
    ...
    // Store speed for collision detection
    ...
    steer.current = THREE.MathUtils.lerp(
      steer.current, steerDir * speedFactor,
      1 - Math.exp(-10 * dt)
    )
    body.setAngvel({ x: 0, y: steer.current * STEER_SPEED * steerSign, z: 0 }, true)

    // ── Camera: zone birds-eye OR follow cam ───────────────────────────────
    ...
    if (zoneCam) {
      // Smooth transition to birds-eye zone view
      ...
      // Follow cam — world-space offset (never rotates with car)
  ...
  return (
    <RigidBody
      ...
      ccd={true}
      restitution={0.2}
      // Rapier collision events — plays crash sound on impact
      onCollisionEnter={() => {
        const spd = lastSpeed.current
        if (spd > 4 && useGameStore.getState().gameStarted) {
          playCollision(spd)
        }
      }}
    >
